...
for z in range(len(target)):
    ...
    ##############################
    # レース別の結果URLをリストで取得
    url = driver.find_elements_by_css_selector('.rp-raceCourse__panel__race__info__buttons__link')
    ...
    for y in range(len(url)):
        ...
        mare = []
        try:
            obj =  driver.find_elements_by_css_selector('.rp-horseTable__pedigreeRow')
