...
# 繰り返し処理時に使用
def r_elem():
    r_elem01 = driver.find_elements(By.CSS_SELECTOR, '#tracks tbody.results:nth-of-type(2) tr td:nth-of-type(2) a:nth-of-type(1)')
    r_elem02 = driver.find_elements(By.CSS_SELECTOR, '#tracks tbody.results:nth-of-type(2) tr td:nth-of-type(3) a:nth-of-type(1)')
    return r_elem01 + r_elem02
# CAPTCHA判別
def captha_check():
    # ページ内のID「contentwrapper」要素を検索
    judg = driver.find_elements(By.CSS_SELECTOR, '#contentwrapper')
    judg = len(judg)
    # 検索した要素があれば次の処理に進む
    if judg > 0:
        pass
...
##対象ページに遷移
driver.get('https://www.equibase.com/static/entry/index.html?SAP=TN')

##2〜5秒待機
time.sleep(ranCount01())
...
############################
# 抽出データを出力するためのCSVを作成
FileName = savePath + 'equibase03_' + str(dateToday) + '.csv'
logFileName = savePath + 'Err_equibase03_' + str(dateToday) + ".txt"
...
# 以下、リンクの数だけ繰り返し処理
for s in range(len(r_target)):

    # 2〜5秒待機
    time.sleep(ranCount01())

    # 「Today」と「Tomorrow」のリンクを再取得
    try:
        r_target = r_elem()
    except:
        err_log('リンク取得エラー')
    ...
    print(str(s) + '/' + str(len(r_target)))

    elemOffset = '$("html,body").animate({scrollTop:$("#tracks").offset().top})'
    driver.execute_script(elemOffset)
    time.sleep(ranCount01())

    # 取得した要素のs番目をクリック
    try:
        ##r_target[s].click()
        targetLink = r_target[s].get_attribute('href')
        print(targetLink)
    ...
    # 「VIEW ALL RACES」をクリック
    try:
        exis = driver.find_element(By.CSS_SELECTOR, '#productButtonDiv').get_attribute('class')
        if 'displayNone' in exis:
            driver.find_element(By.CSS_SELECTOR, '#productButtonDiv')
            driver.find_element(By.XPATH, '//*[@id="lateChangesButtonDiv"]/div[4]/button').click()
        else:
            driver.find_element(By.XPATH, '//*[@id="productButtonDiv"]/div[3]/button').click()
    except:
        err_log('「VIEW ALL RACES」取得エラー')
        driver.quit()
        sys.exit()
    ...
    # Track URL
    trURL = ['?' + driver.find_element(By.CSS_SELECTOR, '.track-name').get_attribute('href').rsplit('?')[-1].strip()]
    # 競馬場を取得
    try:
        trackName = driver.find_element(By.CSS_SELECTOR, '.track-name').text
    except:
        err_log('競馬場 取得エラー')
        trackName = ''
    ...
    # 日付を取得して書式を変換
    try:
        raceDate = driver.find_element(By.CSS_SELECTOR, '.race-date').text
    ...
    # 各レースごとの要素をリストで取得
    try:
        raceBlock = driver.find_elements(By.CSS_SELECTOR, '.entryRace')
    except:
        err_log('個別レース 取得エラー')
        raceBlock = []
    ...
    for l in range(len(raceBlock)):
        ...
        try:
            raceTitle = raceBlock[l].find_element(By.CSS_SELECTOR, 'h4')
        except:
        ...
        # レースNo, 発走時刻, レース格
        try:
            raceLabel = raceTitle.find_elements(By.CSS_SELECTOR, 'span')
        ...
        # レース名
        try:
            raceName = raceTitle.find_element(By.CSS_SELECTOR, '.raceNameLink').text
        except:
            try:
                for i in range(len(raceLabel)):
        ...
        # 距離と馬場種類
        try:
            raceInfo = raceBlock[l].find_element(By.CSS_SELECTOR, '.race-info .entries-race-conditions .conditions-text').text
        ...
        try:
            race_info_head = raceBlock[l].find_elements(By.CSS_SELECTOR, 'table thead tr th')
        ...
        try:
            race_info_table = raceBlock[l].find_elements(By.CSS_SELECTOR, 'table tbody tr')
        except:
        ...
        try:
            for i in range(len(race_info_table)):
                # レコードの1列目を取得
                alsoCheck = race_info_table[i].find_elements(By.CSS_SELECTOR, 'td:nth-of-type(1)')
        ...
        for i in range(len(race_info_table)):
            race_info_tbody = race_info_table[i].find_elements(By.CSS_SELECTOR, 'td')
            ...
            ##血統情報
            try:
                h_data_ttl = race_info_table[i].find_element(By.CSS_SELECTOR, 'td b a').get_attribute('data-original-title')
    ...
    # パンくずの２番目をクリック（Entriesに戻る）
    try:
        driver.find_element(By.CSS_SELECTOR, '.breadcrumbs a:nth-of-type(2)').click()
    except:
        print('ERROR')
